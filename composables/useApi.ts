...
> {
  ...
  const execute = async (ignoreCache = false, ...args: Args): Promise<Data> => {
    ...
    if (!ignoreCache && cache.value.has(cacheKey)) {
      ...
      if (data.fetchedAt && options?.cacheTime) {
        ...
        expirationDate.setTime(
          expirationDate.getTime() + options.cacheTime * 1000,
        );
    ...
    try {
      ...
      let adaptedData = options?.adapter
      ...
      const adaptedError = options?.errorAdapter
        ? (options.errorAdapter(err) as AdaptedError)
        : (err as AdaptedError);
  ...
  const getRef = (defaultValue?: Data, ...args: Args): Ref<Data> => {
    const key = `use-api-${JSON.stringify(args)}`;
    ...
    return computed(() =>
      [...cache.value.entries()]
        .filter(
          ([key]) =>
            index === undefined ||
            JSON.stringify(JSON.parse(key)[index]) === JSON.stringify(arg),
        )
        .map(([, value]) => value),
    );
  };
